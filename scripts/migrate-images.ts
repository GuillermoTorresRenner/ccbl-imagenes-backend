...
class ImageMigrator {
  private readonly csvPath = path.join(process.cwd(), 'img_migrations', 'Metadata.csv');
  private readonly imagesBasePath = path.join(process.cwd(), 'img_migrations', 'imagenes');
  private readonly processedDir = path.join(process.cwd(), 'img_migrations', 'processed');
  private readonly logsDir = path.join(process.cwd(), 'img_migrations', 'logs');
  private readonly reportsDir = path.join(process.cwd(), 'img_migrations', 'reports');
  ...
  async migrate(): Promise<void> {
    ...
    try {
      ...
      console.log(`❌ Registros huérfanos: ${this.migrationLog.orphaned.length}`);
  ...
  private buildImagePath(code: string, folder: string): string {
    ...
    if (code.startsWith('LBCH')) {
      ...
      if (match) {
        ...
        folderName = folder === 'LBCH1' ? 'LBCH1_jpg' : 
                    folder === 'LBCH2' ? 'LBCH2_jpg' :
                    folder === 'LBCH3' ? 'LBCH3_jpg' :
                    folder === 'LBCH4' ? 'LBCH4_jpg' :
                    folder === 'LBCH5' ? 'LBCH5_jpg' : 'LBCH1_jpg';
      }
  ...
  private async validateImageFiles(records: ImageRecord[]): Promise<ImageRecord[]> {
  ...
  private resolveImagePath(relativePath: string): string | null {
    ...
    try {
      const subDirs = fs.readdirSync(this.imagesBasePath, { withFileTypes: true })
        .filter(dirent => dirent.isDirectory())
        .map(dirent => dirent.name);
      
      for (const subDir of subDirs) {
        possiblePaths.push(path.join(this.imagesBasePath, subDir, path.basename(cleanPath)));
      }
    } catch (error) {
  ...
  private async generateProcessedJSON(records: ImageRecord[]): Promise<void> {
    const jsonPath = path.join(this.processedDir, `processed-images-${Date.now()}.json`);
  ...
  private async uploadSingleImage(record: ImageRecord): Promise<void> {
    try {
      ...
      const response = await axios.post(`${this.apiUrl}/api/images/upload`, form, {
        headers: {
          ...form.getHeaders(),
        },
  ...
  private async generateReports(): Promise<void> {
    ...
    const logData = {
      timestamp: new Date().toISOString(),
      summary: {
        total: this.migrationLog.success.length + this.migrationLog.failed.length + this.migrationLog.orphaned.length,
    ...
    const reportPath = path.join(this.reportsDir, `migration-report-${Date.now()}.md`);
...
${summary.failed > 0 ? `
${details.failed.map((item: any, index: number) => 
...
${details.orphaned.slice(0, 20).map((item: any, index: number) => 
...
`;
  ...
  private printSummary(): void {
    const total = this.migrationLog.success.length + this.migrationLog.failed.length + this.migrationLog.orphaned.length;
    ...
    console.log(`📈 Tasa de éxito: ${((this.migrationLog.success.length / (total - this.migrationLog.orphaned.length)) * 100).toFixed(2)}%`);
  }
